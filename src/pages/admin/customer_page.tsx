...
const CustomerPage = () => {
  ...
  const [idCustomer, setIdCustomer] = useState<number | null>(null);
  const [name, setName] = useState<string | null>(null);
  const [phone, setPhone] = useState<string | null>(null);
  const [address, setAddress] = useState<string | null>(null);
  const [idUser, setIdUser] = useState<number | null>(null);

  const itemBreadCrumbs = ["Dashboard", "Data Customer"];
  ...
  return (
    <BaseLayout>
            ...
            <>
              ...
              <div className="flex flex-col w-1/2">
                ...
                <input
                  ...
                  onChange={(e) => setIdCustomer(Number(e.target.value))}
                />
              </div>
            </>
          )}
          ...
          {manage === "view" ? null : (
            <button
              ...
              onClick={() =>
                manage === "add"
                  ? addCustomer()
                  : updateCustomer(idCustomer?.toString()!)
              }
            >
              {manage === "add"
                ? "Tambah"
      ...
      <div className="flex justify-between p-4">
        <Breadcrumb title="Data Customer" items={itemBreadCrumbs} />
        <button
          ...
          <tbody>
            ...
            {customers.map((customer) => (
              <tr key={customer.ID} className="border-b">
                <td className="py-2">{customer.name_customer}</td>
                <td className="py-2">{customer.phone_customer}</td>
                <td className="py-2">{customer.address_customer}</td>
                <td className="py-2">{customer.user.password_user}</td>
                <td className="py-2">{customer.user.username_user}</td>
                <td className="py-2">{customer.user.email_user}</td>
                <td className="py-2 flex space-x-4 justify-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 16 16"
                    width={20}
                    height={20}
                    onClick={() => tapEdit(customer.ID.toString())}
                  >
                  ...
                  <svg
                    ...
                    width={20}
                    height={20}
                    onClick={() => tapView(customer.ID.toString())}
                  >
                    <path
                      fill="#A3A9B6"
                  ...
                  <svg
                    ...
                    width={20}
                    height={20}
                    onClick={() => deleteCustomer(customer.ID.toString())}
                  >
                    <path
                      fill="#A3A9B6"
